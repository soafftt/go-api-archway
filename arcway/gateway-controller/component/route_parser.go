...
func parse(id int, job <-chan RouteSource, result chan<- routeParseResult) {
	...
	if err != nil {
		log.Fatalf("[%d]Initialize policy error:policy unmarshal error: %v", id, err)
	}
...
func (r *routeParser) ParseFromSlice(sources []RouteSource) ([]*dto.UpstreamService, error) {
	...
	for range sources {
		...
		// authorization 이 있으면, jwt key 를 등록한다.
		if result.policy.Authorization != nil {

			gjwt.RegisterKeyByString(result.service, result.policy.Authorization.KeyData, gjwt.JSONKey)
			// 에러 처리
			decodedBytes, err := base64.StdEncoding.DecodeString(result.policy.Authorization.KeyData)
			if err != nil {
				log.Fatalf("authorization decode fail: %v", err)
			}

			// jwt 키 메모리 등록 에러.
			if err := gjwt.RegisterKey(result.service, decodedBytes, gjwt.JSONKey); err != nil {
				log.Fatalf("authorization jwt key register fail: %v", err)
			}
		}
