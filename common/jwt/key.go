...
func RegisterKey(name string, data []byte, keyType KeyType) error {
	keyMu.RLock()
	...
	jwkKey, err := jwk.ParseKey(data, jwk.WithPEM(keyType == PEMKey))
	if err != nil {
		return errors.Join(ErrKeyParse, err)
	}

	entry, err := exportKeyEntry(jwkKey)
	if err != nil {
		return err
	}
...
func exportKeyEntry(jwkKey jwk.Key) (keyEntry, error) {
